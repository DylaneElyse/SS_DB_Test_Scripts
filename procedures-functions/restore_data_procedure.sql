-- ====================================================================
-- SNOWBOARD SCORING DATABASE SEEDING PROGRAM
-- ====================================================================
-- This script defines a series of procedures to reset and populate
-- the database with a complete set of sample data for testing.
--
...
-- --------------------------------------------------------------------
-- PROCEDURE 1: ss_reset_data
-- Purpose: Clears all transactional data from the tables to ensure
...
-- --------------------------------------------------------------------
-- PROCEDURE 2: ss_seed_lookup_tables
-- Purpose: Populates the static lookup tables (roles, disciplines,
-- divisions) with their default values. Uses ON CONFLICT to avoid
...
-- --------------------------------------------------------------------
-- PROCEDURE 5: ss_reseed_heats (DYNAMIC VERSION)
-- Purpose: Dynamically finds all heats that have athletes assigned
-- and calls the reseed_heat procedure for each one. This removes the
-- need for hardcoded heat IDs.
-- --------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE ss_reseed_heats()
LANGUAGE plpgsql
AS $$
DECLARE
    -- A record variable to hold each row returned by the query
    heat_to_reseed RECORD;
BEGIN
    RAISE NOTICE 'Step 5: Dynamically reseeding all heats with registered athletes...';

    -- Loop through every distinct round_heat_id that exists in the heat results table.
    -- This ensures we only try to reseed heats that actually contain athletes.
    -- We order by the ID to ensure the process is repeatable and easier to debug.
    FOR heat_to_reseed IN
        SELECT DISTINCT round_heat_id
        FROM ss_heat_results
        ORDER BY round_heat_id
    LOOP
        -- Call the reseed_heat procedure with the ID found in the loop
...
-- --------------------------------------------------------------------
-- PROCEDURE 6: ss_populate_qualification_scores
-- Purpose: Enters all judge scores for the qualification rounds.
-- Note: PERFORM is used instead of SELECT for functions where the
-- return value is not needed.
-- --------------------------------------------------------------------


-- --------------------------------------------------------------------
-- PROCEDURE 7: ss_populate_finals_scores
-- Purpose: Enters all judge scores for the final rounds.
-- --------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE ss_populate_finals_scores()
LANGUAGE plpgsql
AS $$
BEGIN
    RAISE NOTICE 'Step 7: Populating finals scores...';
    ...
    -- ... (and so on for all finals scores)
...
-- --------------------------------------------------------------------
-- PROCEDURE 8: run_full_database_seed (MASTER PROCEDURE)
-- Purpose: Executes all the seeding and setup procedures in the
-- correct sequence. This is the main entry point for the program.
-- --------------------------------------------------------------------
