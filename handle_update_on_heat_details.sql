...
BEGIN
    IF NEW.round_id IS DISTINCT FROM OLD.round_id THEN
        SELECT event_id, division_id INTO v_old_event_id, v_old_division_id FROM ss_round_details WHERE round_id = OLD.round_id;
        SELECT event_id, division_id INTO v_new_event_id, v_new_division_id FROM ss_round_details WHERE round_id = NEW.round_id;
