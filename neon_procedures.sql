...
BEGIN
    ...
    RAISE NOTICE 'Procedure called for Heat: %, which belongs to Event: %', p_round_heat_id, v_event_id;
    ...
    IF v_personnel_id IS NULL THEN
        ...
        RAISE NOTICE 'Created new event judge with ID: %', v_personnel_id;
    ELSE
        RAISE NOTICE 'Found existing judge with Header "%". Re-using ID: %', p_header, v_personnel_id;
    END IF;
...
BEGIN
    ...
    RAISE NOTICE 'Serpentine seeding completed for Round %.', v_round_id;
END;
$procedure$;
...
BEGIN
    ...
    IF v_source_round_num IS NULL THEN
        RAISE EXCEPTION 'Progression failed: Invalid Round ID %.', p_source_round_id;
    END IF;
    ...
    IF v_num_to_progress IS NULL OR v_num_to_progress <= 0 THEN
        RAISE EXCEPTION 'Progression failed: The destination round (ID: %) has not been configured with the number of athletes to progress to it. Please set number of athletes.', v_destination_round_id;
    END IF;
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE NOTICE 'Reseed Heat: No heat found for Round Heat ID %', p_round_heat_id;
        RETURN;
    END IF;
    ...
    IF v_points_column IS NULL THEN
        RAISE WARNING 'Reseed Heat: Unsupported Discipline ID "%" for heat %. Cannot reseed.', v_discipline_id, p_round_heat_id;
        RETURN;
    END IF;
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE WARNING 'Reseed Heat by Score: No heat found for Round Heat ID %', p_round_heat_id;
        RETURN;
    END IF;
    ...
    IF v_previous_round_id IS NULL THEN
        RAISE WARNING 'Reseed Heat by Score: Could not find previous round for heat %. Seeding will not be performed.', p_round_heat_id;
        RETURN;
    END IF;
